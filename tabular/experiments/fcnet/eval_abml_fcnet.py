...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ABML + FcNet on all test cases.")
    ...
    config = {
        ...
        'gamma_prior_concentration': eval_cfg.get('gamma_prior_concentration', 1.0),
        'gamma_prior_rate': eval_cfg.get('gamma_prior_rate', 0.01),
        'normal_prior_loc': eval_cfg.get('normal_prior_loc', 0.0),
        'normal_prior_scale': eval_cfg.get('normal_prior_scale', 1.0),
    ...
    for tc_path in test_case_files:
        ...
        print("\n" + "="*60)
        print(f"EVALUATION: {tc_name} (Task ID {task_id}) [ABML]")

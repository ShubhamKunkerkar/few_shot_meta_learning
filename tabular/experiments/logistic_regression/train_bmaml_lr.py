...
def main():
    ...
    config = {
        ...
        'svgd_bandwidth_scale': train_cfg.get('svgd_bandwidth_scale', 1.0),
        'svgd_repulsive_weight': train_cfg.get('svgd_repulsive_weight', 1.0),
    ...
    print("Initializing BMAML LR...")
    bmaml = Bmaml(config=config)

    print("Starting training...")
    bmaml.train(train_dataloader=train_dataloader,
    ...
    if os.path.exists(test_path):
        ...
        if len(test_tasks) > 0:
            ...
            bmaml.test(num_eps=min(50, len(test_tasks)), eps_dataloader=test_dataloader)
        else:
            print("\n--- Training complete. (Meta-test split was 0, skipping meta-test) ---")

    print("BMAML LR training completed successfully!")

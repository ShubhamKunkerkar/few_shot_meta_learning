...
def main():
    parser = argparse.ArgumentParser(description="Evaluate MAML + FcNet (Cold Start) on all test cases.")
    ...
    log_dir = os.path.join(REPO_ROOT, 'tabular', 'outputs', 'logs_maml_fcnet')
    ...
    for tc_path in test_case_files:
        ...
        print("\n" + "="*60)
        print(f"EVALUATION (COLD START): {tc_name} (Task ID {task_id}) [MAML FCNet]")

...
def main():
    parser = argparse.ArgumentParser(description="Evaluate PLATIPUS + LogisticRegression (Cold Start) on all test cases.")
    ...
    config = {
        ...
        'minibatch': 1,
        'minibatch_print': 1,
        'logdir': os.path.join(REPO_ROOT, 'tabular', 'outputs', 'logs_platipus_lr'),
        'loss_function': FocalLoss(alpha=loss_cfg.get('alpha', 0.25), gamma=loss_cfg.get('gamma', 2.0))
    }

    print("\nInitializing PLATIPUS and loading pre-trained model (Epoch 20)...")
    ...
    model = platipus.load_model(resume_epoch=config['num_epochs'], hyper_net_class=platipus.hyper_net_class, eps_dataloader=dummy_ep)

    # Base parameters before adaptation (mu_theta prior)
    params_dict = model["hyper_net"].forward()
    phi_zero_shot = [params_dict["mu_theta"]] * config["num_models"]
    ...
    for tc_path in test_case_files:
        ...
        logits_list = platipus.prediction(x=x_v, phi=phi_zero_shot, model=model)
        ...
        print("\n" + "="*60)
        print(f"EVALUATION (COLD START): {tc_name} (Task ID {task_id}) [PLATIPUS LR]")

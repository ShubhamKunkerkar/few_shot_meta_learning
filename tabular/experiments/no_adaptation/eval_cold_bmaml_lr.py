...
def main():
    parser = argparse.ArgumentParser(description="Evaluate BMAML + Logistic Regression (Cold Start) on all test cases.")
    ...
    for tc_path in test_case_files:
        ...
        logits_list = bmaml.prediction(x=x_v, adapted_hyper_net=model["hyper_net"], model=model)
        ...
        print("\n" + "="*60)
        print(f"EVALUATION (COLD START): {tc_name} (Task ID {task_id}) [BMAML LR]")

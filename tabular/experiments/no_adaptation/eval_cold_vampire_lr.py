...
def main():
    parser = argparse.ArgumentParser(description="Evaluate VAMPIRE + LogisticRegression (Cold Start) on all test cases.")
    ...
    config = {
        ...
        'minibatch': 1,
        'minibatch_print': 1,
        'logdir': os.path.join(REPO_ROOT, 'tabular', 'outputs', 'logs_vampire_lr'),
        'loss_function': FocalLoss(alpha=loss_cfg.get('alpha', 0.25), gamma=loss_cfg.get('gamma', 2.0))
    }

    print("\nInitializing VAMPIRE and loading pre-trained model (Epoch 20)...")
    ...
    model = vampire.load_model(resume_epoch=config['num_epochs'], hyper_net_class=vampire.hyper_net_class, eps_dataloader=dummy_ep)
    ...
    for tc_path in test_case_files:
        ...
        logits_list = vampire.prediction(x=x_v, adapted_hyper_net=model["hyper_net"], model=model)
        ...
        print("\n" + "="*60)
        print(f"EVALUATION (COLD START): {tc_name} (Task ID {task_id}) [VAMPIRE LR]")
